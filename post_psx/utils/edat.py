...
class EdatFile(io.RawIOBase):
    ...
    def validate_dev_klic(self, dec_key):
        ...
        # Hash with generated key and compare with dev_hash.
        # key_bytes = key.to_bytes(16, byteorder='little')
        key_bytes = key
        c = CMAC.new(key_bytes, ciphermod=AES)
        c.update(dev[:0x60])
        generated_hash = c.digest()
    ...
    def dec_section(metadata: bytes):
        # Create the 'dec' array by performing the XOR operations as in the original code
        dec = bytearray(16)
    ...
    def decrypt_block(self, block_num, decrypt_key):
        if self.edat_header.flags & self.EDAT_DEBUG_DATA_FLAG != 0 or self.edat_header.flags & self.EDAT_FLAG_0x20 != 0:
        ...
        if self.edat_header.flags & self.EDAT_DEBUG_DATA_FLAG:
            # Decrypt directly if in debug mode
            dec_data = enc_data if not should_decompress else bytearray(enc_data)
        else:
        ...
        if self.edat_header.flags & self.EDAT_COMPRESSED_FLAG and compression_end:
