...
class BlockHashRecord(object):
    ...
    def __str__(self):
        ...
        self.blocknum, " ".join([hex(ord(c)) for c in self.hash]), self.nextblock)
...
class FileListing(object):
    ...
    def __init__(self, data):
        self.filename = data[:0x28].strip(b'\x00')
        assert self.filename != b'', "FileListing has empty filename"
        self.isdirectory = 0x80 & data[0x28] == 0x80
        self.numblocks = struct.unpack("<I", b"%s\x00" % data[0x29:0x29 + 3])[0]  # More little endian madness!
        self.firstblock = struct.unpack("<I", b"%s\x00" % data[0x2F:0x2F + 3])[0]  # And again!
...
class STFS(object):
    ...
    def __init__(self, filename, fd=None):
        """ Takes either a filename to open or a file object (including BytesIO) to parse """
    ...
    def parse_filetable(self):
        ...
        for fl in self.filelistings:  # Build a dictionary to access filelistings by path
            ...
            self.allfiles[b"/".join(path_components)] = fl
            fl.path = b"/".join(path_components)
    ...
    def parse_header(self, data):
        ...
        self.filetable_blocknumber = struct.unpack("<I", b"%s\x00" % data[0x37A + 4:0x37A + 7])[0]  # Why?!?
...
def extract_all(argv):
    if len(argv) < 3:
        print(
        "Usage: stfs.py <input file> <output directory>")
        print(
        "Dumps contents of stfs file to disk")
        return
    s = STFS(argv[1])
    for filename in s.allfiles:  # Loop once creating all the directories
        if s.allfiles[filename].isdirectory:
            print(
            "Creating directory %s" % filename)
            dirpath = filename[1:]
